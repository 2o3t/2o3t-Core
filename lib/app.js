...
const CORE_NAME = require('./constants').CORE_NAME;
...
class AppCore extends KoaApplication {
    ...
    constructor(opts = {}) {
        ...
        const NAME = options.name = process.env.CORE_NAME || options.name || CORE_NAME;
        const logger = options.logger = options.logger || Logger.instance(NAME);
        utils.LOGGER_METHODS.forEach(key => {
            assert(logger[key] && utility.is.function(logger[key]), `loggor.${key} required, and must be a function`);
        });
        ...
        this.ready(() => {
            ...
            Logger.hooks = function(name, args) {
                that.emit('2o3t:logger', name, args);
            };
        });
    ...
    get rootFilePath() {
        ...
        // 获取当前实例的位置
        const rootFilePath = this[OPTIONS].rootFilePath = utility.getCalleeFromStacks(false, 10)[0];
    ...
    get config() {
        return Object.assign({
            name: this[OPTIONS].name || CORE_NAME,
        }, this.loader.config);
    }
