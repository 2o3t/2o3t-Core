...
const isGeneratorFunction = utility.is.generatorFunction;
const co = require('co');
...
class BaseLoader {
     ...
     * @return {Object} obj
     */
    loadServiceFile(filepath, ...inject) {
        const ret = this.loadFile(filepath, ...inject);
        if (ret && utility.is.class(ret)) {
            // 加载service
            const ctx = new ServiceContext(this.app, ...inject);
            return wrapClass(ret, ctx);
        } else if (ret && isFunction(ret)) {
            const ctx = new ServiceContext(this.app, ...inject);
            return ret.bind(ctx);
    ...
    resolveModuleFiles(filepath, dirName = false) {
        ...
        if (fullpath) {
            ...
            const fileList = utility.mapDirsFileBreadth(fileDir, true, 'node_modules');
...
function wrapClass(Clazz, ctx) {
    ...
    while (proto !== Object.prototype) {
        ...
        for (const key of keys) {
            ...
            if (utility.is.function(d.value) && !ret.hasOwnProperty(key)) {
                ret[key] = methodToMiddleware(Clazz, key, ctx);
                ret[key][CLASS_FUNCTION_FULLPATH] = Clazz.prototype[CLASS_FUNCTION_FULLPATH] + '#' + Clazz.name + '.' + key + '()';
                utility.createLazyReadOnlyProp(ret[key], 'name', function() {
                    return Clazz.prototype[CLASS_FUNCTION_FULLPATH] + '#' + Clazz.name + '.' + key + '()';
                });
            }
        }
    ...
    function methodToMiddleware(Clazz, key, ctx) {
        function ContextConstructor(params) {
            Object.keys(params).forEach(key => {
                this[key] = params[key];
            });
        }
        return async function classMiddleware(...args) {
            const clazz = new Clazz();
            ctx.context = new ContextConstructor(clazz);
            const result = await utility.callFn(clazz[key], args, ctx);
            delete ctx.context;
            return result;
        };
    }
