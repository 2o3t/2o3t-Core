...
module.exports = function() {
    ...
    const target = this._loadDirMixins('controller', (controllerDir, controllers, filterFileList) => {
    ...
    //             // filter
    //             const controllersMap = new Map();
    //             target = parseMixins.call(this, controllerDir, controllers, filterFileList)
    //                 .filter(({ name }) => {
    //                 // 去重复
    //                     if (controllersMap.has(name)) {
    //                         this.logger.system.warn(`Controller '${name}' redefined`);
