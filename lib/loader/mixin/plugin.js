...
module.exports = function() {
    ...
    // ZAP: 能力需要增强
    const target = this._loadDirMixinsFromFile(MAIN_NAME, (pluginDir, pluginMX, plugins, filterFileList) => {
        // filter
                        ...
                        if (opts.hasOwnProperty(key)) {
                            ...
                            if (item.mixins && Array.isArray(item.mixins)) {
                                item.mixins.forEach(name => {
                                    if (item[name] && fromModule) {
                                        item[name].module = fromModule;
