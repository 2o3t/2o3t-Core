...
module.exports = {
    ...
    parseEnvConfig(baseDir) {
        ...
        let dotenvPaths = utility.existsFileSync(files, baseDir);
        if (!dotenvPaths || dotenvPaths.length <= 0) { // 找不到会从根目录再找一次
            dotenvPaths = utility.existsFileSync(files, process.cwd());
        }
        const encoding = 'utf8';
